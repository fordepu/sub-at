...
#    EXP=$model\_$datasets\_Fast_SubAT_DLDR
#    EXP1=$model\_$datasets\_Fast_SubAT_Sub
#    DST=new_results/$EXP
#    CUDA_VISIBLE_DEVICES=$device python -u train_adv.py --pgd50  --wandb\
#        --datasets $datasets --attack Fast-AT  --randomseed $seed \
#        --train_eps $eps --test_eps $eps --train_step 1 --test_step 20 \
...
#        --arch=$model --epochs=40  --save-dir=$DST/models --log-dir=$DST --log-name=PSGD --EXP $EXP1
...
#
    ...
    # We suggest use weight decay of 5e-4 instead of 1e-4 (our original setting) for better performance. [1]
    EXP=$model\_$datasets\_PGD_SubAT_DLDR
    EXP1=$model\_$datasets\_PGD_SubAT_Sub
    DST=new_results/$EXP
    CUDA_VISIBLE_DEVICES=$device python -u train_adv.py --pgd50  --wandb\
